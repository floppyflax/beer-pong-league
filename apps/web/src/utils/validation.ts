...
export const matchSchema = matchBaseSchema.refine(
  (m) => m.cups_remaining == null || m.scoreA !== m.scoreB,
  { message: 'cups_remaining only allowed when there is a clear winner (scoreA !== scoreB)', path: ['cups_remaining'] }
);
 ...
 */
export const createMatchInputSchema = matchBaseSchema
  .omit({ id: true })
  .refine(
    (m) => m.cups_remaining == null || m.scoreA !== m.scoreB,
    { message: 'cups_remaining only allowed when there is a clear winner (scoreA !== scoreB)', path: ['cups_remaining'] }
