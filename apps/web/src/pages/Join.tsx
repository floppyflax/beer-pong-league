...
export const Join = () => {
  ...
  const handleScanCode = async (scannedData: string) => {
    setShowScanner(false);
    const parsed = parseQRData(scannedData);

    if (parsed.type === 'tournament_url' && parsed.entityId) {
      navigate(`/tournament/${parsed.entityId}/join`);
      return;
    }
    if (parsed.type === 'league_url' && parsed.entityId) {
      navigate(`/league/${parsed.entityId}`);
      return;
    }
    if (parsed.type === 'code' && parsed.code) {
      try {
        await handleJoinByCode(parsed.code);
      } catch (err) {
        const message =
          ...
          <div className="bg-slate-800 rounded-xl p-6 border border-slate-700/50 text-center">
            ...
            <div className="hidden lg:flex gap-4 mt-6">
              <button
                ...
                className="flex-1 bg-gradient-cta hover:opacity-95 text-white font-bold py-3 px-4 rounded-xl transition-colors flex items-center justify-center gap-3"
