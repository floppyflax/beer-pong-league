 ...
 * (`EloRecalcService.recalculateLeagueElo`) so league_players stats stay
 * coherent. Events without an associated league have no persistent ELO,
 * so the recalc is a no-op there.
 */
...
class MatchAdminService {
  async updateMatch(
    ...
    scoreA: number,
    scoreB: number,
  ): Promise<{ success: boolean; error?: string; leagueId?: string | null; eventId?: string | null }> {
