...
class AuthService {
  ...
  private async signInWithTestAccount(
    ...
    try {
      ...
      if (error) {
        console.warn(
          "⚠️ Password auth failed for test account:",
          error.message,
        );
        ...
        if (otpResult.error) {
          console.error(
            "❌ OTP fallback also failed:",
            otpResult.error.message,
          );
          return {
            error: new Error(
              "Password auth not enabled. Please check your email for OTP link.",
            ),
            usedOTP: true,
          };
        }
      ...
      console.log(
        "🧪 Test account logged in successfully with password:",
        email,
      );
