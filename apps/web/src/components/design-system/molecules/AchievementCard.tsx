 ...
 *
 * Phase D.3 — Beer Pong ELO redesign. Uses only Tailwind tokens from the
 * Everything ELO palette.
 */
...
function AchievementIcon({ iconKey }: { iconKey: string }) {
  ...
  switch (iconKey) {
    ...
    case "star":
      return <Star size={size} className={cls} />;
    case "flame":
      return <Flame size={size} className={cls} />;
    default:
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] font-mono text-cool-gray/60 mt-0.5">
          {earnedDate}
        </p>
      </div>
    </div>
