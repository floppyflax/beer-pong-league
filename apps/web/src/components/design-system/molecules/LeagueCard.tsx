/**
 * LeagueCard — pressed-card listing for a league.
 *
 * Aligned with `EventCard` visually. Header status pill (Active/Terminée +
 * Owner badge), title, then a 3-stat row (members, events, last activity).
 */
...
export const LeagueCard: React.FC<LeagueCardProps> = ({ league }) => {
  ...
  return (
    <button
      ...
      className="w-full text-left bg-navy-soft border border-card rounded-card p-4 hover:border-cool-gray transition-colors duration-75"
    >
