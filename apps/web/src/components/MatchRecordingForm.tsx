...
import { PhotoService, type PhotoResult } from "@/services/PhotoService";
...
interface MatchRecordingFormProps {
  ...
  participants: Player[];
  onSuccess: (match: Match & { cupsRemaining?: number; photoBlob?: Blob }) => void;
  onClose: () => void;
}
...
export function MatchRecordingForm({
  ...
  const [cupsRemaining, setCupsRemaining] = useState<number | null>(null);
  ...
  const handlePickPhoto = async () => {
    try {
      ...
      toast.error("Impossible de sélectionner la photo");
    }
  };
  ...
  const validateForm = (): boolean => {
    ...
    // Validate team 1 players (design-system 7.4: Équipe 1)
    if (teamA.length !== playersPerTeam || teamA.some((id) => !id)) {
      newErrors.teamA = `Sélectionnez ${playersPerTeam} joueur${playersPerTeam > 1 ? "s" : ""} pour l'équipe 1`;
    }

    // Validate team 2 players (design-system 7.4: Équipe 2)
    if (teamB.length !== playersPerTeam || teamB.some((id) => !id)) {
      newErrors.teamB = `Sélectionnez ${playersPerTeam} joueur${playersPerTeam > 1 ? "s" : ""} pour l'équipe 2`;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const scoreA = winner === "A" ? GAME_MAX_SCORE : 0;
      const scoreB = winner === "B" ? GAME_MAX_SCORE : 0;

      const match: Match & { cupsRemaining?: number; photoBlob?: Blob } = {
        ...
        scoreB,
        status: "pending",
        ...(cupsRemaining != null && { cupsRemaining }),
        ...(photo && { photoBlob: photo.blob }),
      };
  ...
  return (
    <div className="fixed inset-0 bg-black/90 z-50 flex flex-col p-4 overflow-y-auto">
      ...
      <form onSubmit={handleSubmit} className="flex-grow space-y-6">
        {/* Équipe 1 - design-system 7.4 */}
        <div className="bg-slate-800 rounded-lg p-6">
          <h3 className="text-xl font-bold text-white mb-4">Équipe 1</h3>
        ...
        {/* Équipe 2 - design-system 7.4 */}
        <div className="bg-slate-800 rounded-lg p-6">
          <h3 className="text-xl font-bold text-white mb-4">Équipe 2</h3>
        ...
        {/* Cups remaining (optional) */}
        {winner && (
          <Card variant="default" padding="md">
            <label className="block text-sm font-bold text-slate-400 mb-3">
              Gobelets restants de l'équipe gagnante
            </label>
            <div className="flex items-center justify-center gap-4">
              <button
                type="button"
                onClick={() => setCupsRemaining((prev) => prev != null ? Math.max(MIN_CUPS, prev - 1) : MAX_CUPS)}
              ...
              <span className="text-3xl font-bold text-primary min-w-[3rem] text-center">
                {cupsRemaining ?? "—"}
              </span>
              <button
                type="button"
                onClick={() => setCupsRemaining((prev) => prev != null ? Math.min(MAX_CUPS, prev + 1) : MIN_CUPS)}
                className="w-10 h-10 rounded-full bg-slate-700 flex items-center justify-center text-white hover:bg-slate-600 transition-colors"
            ...
            {cupsRemaining != null && (
              <button
                type="button"
                onClick={() => setCupsRemaining(null)}
                className="mt-2 text-xs text-slate-500 hover:text-slate-400 mx-auto block"
              >
                Ne pas renseigner
              </button>
            )}
            <p className="text-xs text-slate-500 text-center mt-2">
              Optionnel — Combien de gobelets restait-il ?
            </p>
          </Card>
        )}

        {/* Photo finish (optional) */}
        {winner && (
          <Card variant="default" padding="md">
            <label className="block text-sm font-bold text-slate-400 mb-3">
              Photo finish
            ...
            <p className="text-xs text-slate-500 text-center mt-2">
              Optionnel — Immortalisez le moment !
            </p>
          </Card>
        )}
