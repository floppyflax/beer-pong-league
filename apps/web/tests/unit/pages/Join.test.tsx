...
vi.mock("../../../src/components/navigation/BottomMenuSpecific", () => ({
  BottomMenuSpecific: ({
    ...
    actions: BottomMenuAction[];
    variant?: string;
  }) => (
    <div data-testid="bottom-menu" data-variant={variant}>
...
vi.mock("../../../src/components/join/QRScanner", () => ({
  QRScanner: ({ onClose, onScan, onFallbackToCodeInput }: QRScannerProps) => (
...
vi.mock("../../../src/components/join/CodeInputModal", () => ({
  CodeInputModal: ({ onClose, onSubmit }: CodeInputModalProps) => (
...
describe("Join Page", () => {
  ...
  describe("AC1: Page Layout", () => {
    ...
    it("should render bottom menu with 2 actions", () => {
      ...
      expect(
        within(bottomMenu).getByRole("button", { name: /scanner qr/i }),
      ).toBeInTheDocument();
      expect(
        within(bottomMenu).getByRole("button", { name: /code/i }),
      ).toBeInTheDocument();
    });

    it("should render HelpCard with Comment ça marche", () => {
      renderJoinPage();
  ...
  describe("AC2: QR Scanner Modal", () => {
    it('should open QR scanner when "SCANNER QR" is clicked', () => {
      ...
      const scannerButton = within(screen.getByTestId("bottom-menu")).getByRole(
        "button",
        { name: /scanner qr/i },
      );
  ...
  describe("AC3: Code Input Modal", () => {
    it('should open code input modal when "CODE" is clicked', () => {
      ...
      const codeButton = within(screen.getByTestId("bottom-menu")).getByRole(
        "button",
        { name: /code/i },
      );
    ...
    it("should handle code submission", async () => {
      ...
      const bottomMenu = screen.getByTestId("bottom-menu");
      const codeButton = within(bottomMenu).getByRole("button", {
        name: /code/i,
      });
      fireEvent.click(codeButton);
      ...
      await waitFor(() => {
        expect(
          screen.queryByTestId("code-input-modal"),
  ...
  describe("AC6: Desktop Experience", () => {
    it("should render desktop action buttons", () => {
      ...
      expect(
        screen.getAllByRole("button", { name: /scanner qr/i }).length,
      ).toBeGreaterThan(0);
    });
  });
  ...
  describe("Design system (Story 14-23)", () => {
    it('should pass variant="gradient" to BottomMenuSpecific', () => {
      renderJoinPage();
      const bottomMenu = screen.getByTestId("bottom-menu");
      expect(bottomMenu).toHaveAttribute("data-variant", "gradient");
    });

    it("should show toast and open code modal when scan join fails", async () => {
      mockJoinByCode.mockRejectedValueOnce(new Error("Code invalide"));
      ...
      expect(toast.error).toHaveBeenCalledWith("Code invalide");
    });
  });
});
