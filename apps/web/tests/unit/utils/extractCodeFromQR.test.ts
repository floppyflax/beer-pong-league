...
describe('extractCodeFromQR', () => {
  it('should extract code from URL with query param', () => {
    expect(extractCodeFromQR('https://bpl.com/event/123?code=ABC123')).toBe('ABC123');
  ...
  it.skip('should extract code from path segment when valid format', () => {
    expect(extractCodeFromQR('/event/join/ABC123')).toBe('ABC123');
    expect(extractCodeFromQR('https://bpl.com/event/123/ABCD1234')).toBe('ABCD1234');
  });
  ...
  it('should return empty string for invalid QR (no valid 6-8 char code)', () => {
    expect(extractCodeFromQR('https://example.com/event/123/join')).toBe('');
