...
describe('MatchRecordingForm - Story 5.1', () => {
  const mockParticipants: Player[] = [
    { id: 'player-1', name: 'Alice', elo: 1500, wins: 0, losses: 0, matchesPlayed: 0, streak: 0 },
    { id: 'player-2', name: 'Bob', elo: 1500, wins: 0, losses: 0, matchesPlayed: 0, streak: 0 },
    { id: 'player-3', name: 'Charlie', elo: 1500, wins: 0, losses: 0, matchesPlayed: 0, streak: 0 },
    { id: 'player-4', name: 'Diana', elo: 1500, wins: 0, losses: 0, matchesPlayed: 0, streak: 0 },
  ];
  ...
  describe('Task 2: Implement player selection', () => {
    ...
    it('should prevent duplicate player selection between teams', async () => {
      ...
      await waitFor(() => {
        ...
        const aliceOption = teamBOptions.find(opt => opt.textContent === 'Alice');
        expect(aliceOption).not.toBeInTheDocument() || expect(aliceOption).toHaveAttribute('disabled');
      });
    });
    ...
    it('should prevent duplicate player selection within same team', async () => {
      ...
      await waitFor(() => {
        const secondOptions = Array.from(teamASecondSelect.querySelectorAll('option'));
        const aliceOption = secondOptions.find(opt => opt.textContent === 'Alice');
        expect(aliceOption).not.toBeInTheDocument() || expect(aliceOption).toHaveAttribute('disabled');
      });
    });
  });
  ...
  describe('Task 3: Implement score entry', () => {
    it('should have score inputs with default values', () => {
      ...
      const scoreAInput = screen.getByLabelText(/score équipe a/i) as HTMLInputElement;
      const scoreBInput = screen.getByLabelText(/score équipe b/i) as HTMLInputElement;
  ...
  describe('Task 4: Add format support', () => {
    it('should detect tournament format and adjust player slots', () => {
      const { rerender } = render(<MatchRecordingForm {...defaultProps} format="1v1" />);
  ...
  describe('Task 5: Integrate Zod validation', () => {
    ...
    it('should validate scores', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/équipe doit avoir 10 points/i)).toBeInTheDocument();
      });
    });
    ...
    it('should display validation errors', async () => {
      ...
      await waitFor(() => {
        const errorMessages = screen.queryAllByText(/sélectionnez|score|joueur/i);
