...
describe('PaymentModal', () => {
  ...
  describe('Task 2: Premium Benefits Display', () => {
    ...
    it('should display benefit descriptions', () => {
      ...
      expect(screen.getByText(/Aucune limite de participants/)).toBeInTheDocument();
    });
  });
  ...
  describe('Task 5: Payment States', () => {
    ...
    it('should show success state after payment', async () => {
      ...
      render(<PaymentModal isOpen={true} onClose={mockOnClose} onSuccess={mockOnSuccess} />);
  ...
  describe('Task 6: Payment Success Handling', () => {
    ...
    it('should update localStorage on success', async () => {
      ...
      await waitFor(() => {
        expect(premiumService.updatePremiumStatusInLocalStorage).toHaveBeenCalledWith(true);
      });
    });
  ...
  describe('Task 7: Payment Error Handling', () => {
    ...
    it('should handle authentication error', async () => {
      ...
      const { rerender } = render(<PaymentModal isOpen={true} onClose={mockOnClose} />);
  ...
  describe('Task 8: Modal Close Handling', () => {
    ...
    it('should not close modal when clicking continue in confirmation', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByText('Annuler le paiement ?')).not.toBeInTheDocument();
    ...
    it('should prevent close during success state', async () => {
      vi.mocked(premiumService.isPremium).mockResolvedValue(true);
      ...
      // Close button should be disabled in success state
      const closeButton = screen.queryByLabelText('Fermer');
      expect(closeButton).toBeNull(); // Not present in success view
    });
  });
  ...
  describe('Edge Cases and Bug Fixes', () => {
    it('FIX #4: should prevent double-click payment', async () => {
      ...
      await waitFor(() => {
        expect(premiumService.updatePremiumStatusInLocalStorage).toHaveBeenCalledTimes(1);
      });
    });
    ...
    it('FIX #1 & #3: should cleanup timers on unmount during processing', async () => {
      ...
      const { unmount } = render(<PaymentModal isOpen={true} onClose={mockOnClose} onSuccess={mockOnSuccess} />);
    ...
    it('FIX #7: should handle polling timeout after 10 seconds', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/n'a pas pu être confirmé/)).toBeInTheDocument();
      });
    });
