...
const mockAddAnonymousPlayerToTournament = vi.fn().mockResolvedValue('new-player-id');
...
describe('TournamentJoin - Story 4.1', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockEnsureIdentity.mockResolvedValue({ type: 'anonymous', anonymousUserId: 'test-anon-id' });
  });
  ...
  describe('Task 1: Review TournamentJoin page (AC: Page opens)', () => {
    it('should render the page when tournament exists', async () => {
      ...
      // AC: Page opens and renders correctly
      expect(screen.getByText(/rejoindre le tournoi/i)).toBeInTheDocument();
      expect(screen.getByText('Test Tournament')).toBeInTheDocument();
    });

    it('should load tournament data from URL parameter', async () => {
      render(<TournamentJoin />, { wrapper: Wrapper });

      // AC: Tournament data is fetched
      await waitFor(() => {
        expect(screen.getByText('Test Tournament')).toBeInTheDocument();
      });
    });
  ...
  describe('Task 2: Implement join form (AC: Enter name)', () => {
    ...
    it('should display simple name input form', () => {
      render(<TournamentJoin />, { wrapper: Wrapper });
    ...
    it('should validate name minimum length (1 char)', async () => {
      ...
      const createButton = screen.getByText(/créer un nouveau joueur/i);
      ...
      // AC: Validate name (min 1 char)
      await waitFor(() => {
        expect(toast.default.error).toHaveBeenCalledWith(expect.stringContaining('vide'));
      });
    });
  ...
  describe('Task 3: Create anonymous user if needed (AC: Anonymous user created)', () => {
    ...
    it('should create anonymous user if user has no identity', async () => {
      mockEnsureIdentity.mockResolvedValue({ type: 'anonymous', anonymousUserId: 'new-anon-id' });
    ...
    it('should not create new identity if user already has one', async () => {
      mockEnsureIdentity.mockResolvedValue({ type: 'anonymous', anonymousUserId: 'existing-anon-id' });
